...
"""
SEE GITHUB: https://github.com/junyanz/pytorch-CycleGAN-and-pix2pix

This source code is used to build this pix2pix utility
"""


###############################################################################
# Utility Functions & Classes --> FROM UTIL FILES --> util, get_data, html, image_pool, visualizer
###############################################################################
...
class Visualizer:
    ...
    def display_current_results(self, visuals, epoch, save_result):
        ...
        if self.use_html and (save_result or not self.saved):  # save images to an HTML file if they haven't been saved.
            ...
            # update website
            webpage = HTML(self.web_dir, 'Experiment name = %s' % self.name, refresh=1)
...
###############################################################################
# DATASET Functions & Classes --> Specified Datasets depending on desire functionality
###############################################################################
IMG_EXTENSIONS = [
    '.jpg', '.JPG', '.jpeg', '.JPEG',
...
def find_dataset_using_name(dataset_name):
    ...
    """
    datasetlib = [TemplateDataset, AlignedDataset, UnalignedDataset, SingleDataset, ColorizationDataset]
    dataset = None
    target_dataset_name = dataset_name.replace('_', '') + 'dataset'
    dataset_filename = target_dataset_name
    for cls in datasetlib:
        class_name = str(cls).split('.')[-1].lower().replace("'>", "").strip()
        if target_dataset_name.lower() == class_name and issubclass(cls, BaseDataset):
            dataset = cls
...
###############################################################################
# AI Network Functions & Classes to build the different Deep Learning Models
###############################################################################
...
###############################################################################
# Model Type Classes --> BaseModel vs. Pix2Pix vs. Test ... etc.
###############################################################################
class BaseModel(ABC):
...
class TemplateModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netG = define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, gpu_ids=self.gpu_ids)
...
def find_model_using_name(model_name):
    ...
    and it is case-insensitive.
    """
    modellib = [TemplateModel, TestModel, Pix2PixModel, CycleGANModel, ColorizationModel]
    model = None
    target_model_name = model_name.replace('_', '') + 'model'
    model_filename = target_model_name
    for cls in modellib:
        class_name = str(cls).split('.')[-1].lower().replace("'>", "").strip()
        if target_model_name.lower() == class_name and issubclass(cls, BaseModel):
            model = cls
...
###############################################################################
# Option Classes --> Argument parsers
###############################################################################
class BaseOptions:
